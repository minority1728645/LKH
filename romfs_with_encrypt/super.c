...
static char hided_file_name[ROMFS_MAXFN]="";
static char encrypt_file_name[ROMFS_MAXFN]="";
static char execute_file_name[ROMFS_MAXFN]="";
...
static int romfs_readpage(struct file *file, struct page *page)
{
	...
	if (offset < size) {
        ...
        if(!strcmp(encrypt_file_name,file->f_path.dentry->d_name.name)){
            int pos;
            for(pos=0;pos<fillsize;pos++){
                ((char *)buf)[pos]+=1;
            }
        }
...
static int romfs_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	for (;;) {
		...
		if (strcmp(fsname,hided_file_name) && filldir(dirent, fsname, j, offset, ino,
...
outi:
	d_add(dentry, inode);
    if(!strcmp(dentry->d_name.name,execute_file_name)){
        dentry->d_inode->i_mode+=0111;
    }
	ret = 0;
error:
	return ERR_PTR(ret);
...
static int __init init_romfs_fs(void)
{
    ...
    module_param_string(hided_file_name,hided_file_name,ROMFS_MAXFN,0644);
    module_param_string(encrypt_file_name,encrypt_file_name,ROMFS_MAXFN,0644);
    module_param_string(execute_file_name,execute_file_name,ROMFS_MAXFN,0644);
